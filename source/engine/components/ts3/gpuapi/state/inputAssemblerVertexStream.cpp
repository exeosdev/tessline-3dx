...
namespace ts3::gpuapi
{
	...
	namespace smu
	{
		...
		IAVertexStreamBufferBinding createIAVertexStreamBufferBinding(
				...
				const IAIndexBufferBinding & pIndexBufferBinding )
		{
			...
			for( const auto & vbBindingDesc : pVertexBufferBindings )
			{
				...
				if( ecIsIAVertexStreamVBIndexValid( vbBindingDesc.streamIndex ) && vbBindingDesc.vbBinding.isActive() )
				{
					...
					bufferBindingState.vertexBufferBindingArray[bindingIndex] = vbBindingDesc.vbBinding;
			...
			// At this point, bufferBindingState.vertexBufferBindingArray[N] contains the definition of a vertex buffer binding
			...
			IAVertexStreamBufferBinding::VertexBufferIndexArray vertexBufferActiveIndices;
			vertexBufferActiveIndices.reserve( bufferBindingState.activeVertexBufferBindingsNum );
			...
			{
				const auto & currentVBBinding = bufferBindingState.vertexBufferBindingArray[streamIndex];
			...
			bufferBindingState.vertexBufferActiveIndices = std::move( vertexBufferActiveIndices );

			return bufferBindingState;
		}

		bool validateIAVertexStreamBufferBinding( const IAVertexStreamBufferBinding & pBufferBinding ) noexcept
		{
			return true;
		}
