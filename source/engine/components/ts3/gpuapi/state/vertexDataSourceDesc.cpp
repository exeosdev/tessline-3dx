...
namespace ts3::gpuapi
{
	...
	VertexDataSourceBinding createVertexDataSourceBinding( const VertexDataSourceBindingDesc & pBindingDesc )
	{
		...
		// At this point, dataSourceBinding.vertexBufferBindingArray[N] contains the definition of a binding
		// (offset, stride and the buffer itself) for a stream index N. That array may have gaps which is
		// perfectly fine - this is allowed to enable better flexibility and ability to replace IB/VB bindings
		// without switching other heavy pipeline state.
		...
		//   (i.e. those with a vertex buffer bound), represented by their indices. This list is used by the older API
		...
		//   For example, if user specifies three VBOs bound to stream 0, 1 and 4, two ranges will be produced:
		...
		{
			const auto & vbBinding =  dataSourceBinding.vertexBufferBindingArray[streamIndex];
			...
			if( !vbBinding.bufferObject || ( streamIndex + 1 == E_GPU_SYSTEM_METRIC_IA_MAX_VERTEX_INPUT_STREAMS_NUM ) )
			{
