...
namespace ts3
{
	...
	bool SysDisplayManager::checkDisplayDriverSupport( ESysDisplayDriverType pDriverID )
	{
		switch( pDriverID )
		{
	...
	ESysDisplayDriverType SysDisplayManager::resolveDisplayDriverID( ESysDisplayDriverType pDriverID )
	{
		ESysDisplayDriverType resolvedDriverID = ESysDisplayDriverType::Unknown;

		if( pDriverID == ESysDisplayDriverType::Generic )
		{
			resolvedDriverID = ESysDisplayDriverType::Generic;
		}
		else if( pDriverID == ESysDisplayDriverType::Default )
		{
		#if( TS3_SYSTEM_DISPLAY_DRIVER_SUPPORT_DXGI )
			resolvedDriverID = ESysDisplayDriverType::DXGI;
		#else
			resolvedDriverID = ESysDisplayDriverType::Generic;
		#endif
		}
	#if( TS3_SYSTEM_DISPLAY_DRIVER_SUPPORT_DXGI )
		else if( pDriverID == ESysDisplayDriverType::DXGI )
		{
			resolvedDriverID = ESysDisplayDriverType::DXGI;
		}
	#endif
