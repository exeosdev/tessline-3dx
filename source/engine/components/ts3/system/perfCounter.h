...
namespace ts3
{
	...
	using sys_perf_counter_res_t = uint64;

	/// @brief Type used to represent values of stamps generated by a PerfCounter.
	using sys_perf_counter_value_t = uint64;


	class SysPerfCounter
	{
	public:
		...
		/// would be done by calling: pfcConvertToDuration( timeStampDeff, std::make_pair( 1, 1000000000 ) );
		static long double convertToDuration( sys_perf_counter_value_t pStampDiff, const std::pair<native_int, native_int> & pUnitRatio );

		/// @brief Converts a duration value back to a PC time stamp difference using specified unit ratio.
		/// Assertion: pfcConvertFromDuration( pfcConvertToDuration( timeStampDeff, R ), R ) == timeStampDeff for any given R.
		static sys_perf_counter_value_t convertFromDuration( long double pDuration, const std::pair<native_int, native_int> & pUnitRatio );
		...
		static long double convertToDuration( sys_perf_counter_value_t pStampDiff )
		{
		...
		template <DurationPeriod tpPeriod>
		static sys_perf_counter_value_t convertFromDuration( const Duration<tpPeriod> & pDuration )
		{
