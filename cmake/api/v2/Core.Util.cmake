...
function( ebsCoreUtilListAppend pListName pContent )
...
#@DocStr -- XCACHE:     the output list is set in 'CACHE INTERNAL' mode.
function( ebsCoreUtilListRemove pListName pContent )
...
#@FunDef ebsCoreUtilListCheckContent
...
function( ebsCoreUtilListCheckContent pListName pContent outResult )
    if( NOT pContent )
        return()
    endif()
...
#@FunDef ebsCoreUtilStringReplace
#@DocStr Replaces matched string content with another string.
function( ebsCoreUtilStringReplace pString pMatch pReplace outResult )
    if( NOT pString )
        return()
    endif()
...
#@FunDef ebsCoreUtilStringSplitEx
#@DocStr Splits string into a list of substrings using specified separator.
function( ebsCoreUtilStringSplitEx pString pSeparator outList outLength )
    if( NOT pString )
        return()
    endif()
    ...
    # Handle the optional list length retrieval
    if( outLength )
        list( LENGTH result listLength )
        set( ${outLength} ${listLength} PARENT_SCOPE )
    endif()
endfunction()

#@FunDef ebsCoreUtilStringSplit
#@DocStr Splits string into a list of substrings using specified separator.
macro( ebsCoreUtilStringSplit pString pSeparator outList )
    ebsCoreUtilStringSplitEx( ${pString} ${pSeparator} ${outList} FALSE )
endmacro()

#@FunDef ebsCoreUtilPlatformQuerySystemName
...
function( ebsCoreUtilSysQueryCMakeSystemName outResult )
